...
```{r}
lm.plot = ggplot(data = data.frame(cbind(X,Y)), aes(x=X1, y=X2)) +  # combine X, Y into data frame, plot Y vs X
            geom_point() +                                          # represent each data point as a point
            geom_abline(intercept = 10, slope = 2) +                # plot a line showing true Y~X relationship
            geom_smooth(method=lm, se=FALSE, color="red") +         # plot  the linear regression line
            xlab("X") + ylab("Y")
lm.plot
```
...
```{r}
beta = as.matrix(expand.grid(x=seq(7,13,by=0.3), y = seq(1.7,2.3, by=0.1)))  #generate matrix of beta vectors
Y.broadcast = matrix(Y, nrow=N, ncol=dim(beta)[1])

RSS = diag(t(Y.broadcast-X.int%*%t(beta)) %*% (Y.broadcast-X.int%*%t(beta))) #calculate RSS for each beta
...
RSS.plot = ggplot(data = RSS.data, aes(x=beta0, y=beta1, z = RSS, fill=RSS)) +
                    ...
                    geom_point(x=coef(lm.fit)[1], y=coef(lm.fit)[2], size=3, color="white") +
                    scale_fill_viridis()
RSS.plot
```
...
beta.tprob = 2*pt(q=beta.tstat, df=(N-p-1), lower.tail=FALSE) #get 2-tailed probability from t distribution
...
beta.fprob = pf(q=beta.fstat, df1 = p-0, df2 = N-p-1, lower.tail=FALSE) #get probabilty from F-distribution
